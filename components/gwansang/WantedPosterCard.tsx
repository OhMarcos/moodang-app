...
function drawParchmentBg(ctx: CanvasRenderingContext2D) {
  ...
  // Coffee/tea stain (random placement)
...
/**
 * 조선시대 수배령 (Wanted Poster) — 1080x1080
 *
 * Layout:
 *   ┌─────────────────────────┐
 *   │   ═══ 수  배  령 ═══    │  ← Header with hanja
 *   │      "수배" + headline   │
 *   │  ┌───────────────────┐  │
 *   │  │                   │  │
 *   │  │    PHOTO          │  │  ← Mugshot with ink-wash filter
 *   │  │    (woodblock)    │  │
 *   │  │                   │  │
 *   │  └───────────────────┘  │
 *   │   죄목: ...              │  ← Crime (harsh truth)
 *   │   현상금: XX냥           │  ← Bounty (charm score)
 *   │  ─── ◇ ───              │
 *   │   능력치 5개 요약        │  ← Stats summary
 *   │        [관인]            │  ← Red stamp/seal
 *   │   moodang.app            │
 *   └─────────────────────────┘
 */
function drawWantedPoster(
  ...
  ctx.fillStyle = P.ink;
  ctx.fillText("수 배 령", s / 2, curY);

  // Hanja subtitle
  curY += 34;
  ctx.font = `400 26px "Noto Serif KR", serif`;
  ctx.fillStyle = P.inkFaded;
  ctx.fillText("搜  捕  令", s / 2, curY);

  // Decorative line under title
  curY += 20;
  drawDivider(ctx, curY, PAD + 40, s - PAD - 40);

  // ─── Headline (nickname-based) ───
  curY += 40;
  const nickname = reading.funTags?.nickname ?? "관상 분석 대상";
  // Strip "~의 상이로구나" for wanted poster headline
  ...
  ctx.fillStyle = P.red;
  ctx.textAlign = "center";
  ctx.fillText(`수배: ${headline}`, s / 2, curY);
  ...
  // Apply mugshot filter
  applyMugshotFilter(ctx, photoX, photoY, photoW, photoH);
  ctx.restore();

  // Photo corner marks (mugshot style)
  ...
  // 죄목 (Crime) — use first harsh truth
  const crime = reading.harshTruths?.truths?.[0] ?? "사람을 홀리는 관상";
  const crimeShort = crime.length > 25 ? crime.slice(0, 25) + "..." : crime;

  ctx.font = `bold 28px "Noto Serif KR", serif`;
  ctx.textAlign = "center";
  ctx.fillStyle = P.ink;
  ctx.fillText(`죄   목`, s / 2, curY);
  ...
  // 현상금 (Bounty) — use charm score
  curY += 36;
  const bounty = reading.funTags?.charmScore ?? reading.viralScores.sexAppeal;

  ctx.font = `bold 28px "Noto Serif KR", serif`;
  ctx.fillStyle = P.ink;
  ctx.fillText("현 상 금", s / 2, curY);

  curY += 42;
  ctx.font = `bold 52px "Noto Serif KR", serif`;
  ctx.fillStyle = P.red;
  ctx.fillText(`${bounty} 냥`, s / 2, curY);

  // Small note under bounty
  curY += 22;
  ctx.font = `400 16px "Noto Serif KR", serif`;
  ctx.fillStyle = P.inkFaded;
  ctx.fillText("(생포 시 두 배)", s / 2, curY);

  // ─── Stats summary (compact bottom bar) ───
  curY += 32;
  drawDivider(ctx, curY, PAD + 40, s - PAD - 40);
  curY += 28;
  ...
  const statLabels = [
    { label: "색기", value: scores.sexAppeal },
    { label: "총기", value: scores.sharpMind },
    { label: "재력", value: scores.wealthPotential },
    { label: "인복", value: scores.peopleLuck },
    { label: "관종", value: scores.mainCharacterEnergy },
  ];

  const totalStatsW = statLabels.length * 100 + (statLabels.length - 1) * 20;
  ...
  for (const stat of statLabels) {
    ...
    ctx.textAlign = "center";
    ctx.fillStyle = P.inkFaded;
    ctx.fillText(stat.label, statX + 50, curY);

    // Value
    ctx.font = `bold 28px "Noto Serif KR", serif`;
    ctx.fillStyle = stat.value >= 70 ? P.red : stat.value >= 40 ? P.ink : P.inkFaded;
    ctx.fillText(`${stat.value}`, statX + 50, curY + 32);

    statX += 120;
  }

  // ─── Red official stamp (관인) ───
  const stampX = s * 0.78;
  ...
  // Stamp text (2 characters vertically)
  ctx.font = `bold 32px "Noto Serif KR", serif`;
  ctx.fillStyle = P.redStamp;
  ctx.textAlign = "center";
  ctx.textBaseline = "middle";
  ctx.fillText("官", stampX, stampY - 14);
  ctx.fillText("印", stampX, stampY + 18);

  // Slight rotation for realism — apply worn effect
  ctx.globalAlpha = 0.15;
  ctx.beginPath();
  ctx.arc(stampX + 3, stampY + 2, stampR - 4, 0, Math.PI * 2);
  ctx.fillStyle = P.redStamp;
  ctx.fill();
...
const WantedPosterCard = forwardRef<WantedCardHandle, WantedPosterCardProps>(
  function WantedPosterCard({ imageUrl, reading }, ref) {
    ...
    useEffect(() => {
      ...
      img.onload = () => {
        canvas.width = SIZE;
        canvas.height = SIZE;
        drawWantedPoster(ctx, img, reading);
        setIsReady(true);
      };
